...
class invoicr extends FPDF_rotation 
{
	...
	var $margins = array('l'=>20,'t'=>20,'r'=>20, 'd'=> 20);
	...
	*******************************************************************************/
	function invoicr($size='A4',$currency='€',$language='en')
	{
		...
		$this->AliasNbPages();
		$this->SetMargins($this->margins['l'],$this->margins['t'],$this->margins['r'], $this->margins['d']);
	}
	...
	function setqrcode($qrcode=0,$maxWidth=0,$maxHeight=0)
	{
		if($maxWidth and $maxHeight) {
			$this->maxImageDimensions = array($maxWidth,$maxHeight);
		}
		...
		$this->qrdimensions = $this->resizeToFit($qrcode);
	}
	...
	*******************************************************************************/
	...
	{	
		...
		if(isset($this->qrcode)) {
			$this->Image($this->qrcode,$this->margins['l'],$this->GetY(),$this->qrdimensions[0],$this->qrdimensions[1]);
		}
		$this->Ln(4);
	...
	function Footer()
	{
		...
		$this->SetFont($this->font,'',8);
		$this->SetTextColor(50,50,50);
		$this-> Image($this->logo,100,300,300,300);
	...
	function PaynowCode()
	{
		$this->Image($this->$logo,$this->getX(),$this->getY(),$this->dimensions[0],$this->dimensions[1]);
	}
	/*******************************************************************************
	*                                                                              *
	*                               Private methods                                *
